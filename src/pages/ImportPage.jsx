      ...
      {result && (
        <>
          <Alert type={result.success ? 'success' : 'error'}>
            {result.success
              ? `✓ Importación completada — hoja "${result.sheet}"`
              : `⚠ Importación con errores — "${result.sheet}"`}
          </Alert>
          ...
          <div className="stats-row">
            {[
              ...
              { v: result.assets_unchanged ?? 0, l: 'Assets sin cambios', c: '' },
          ...
          {result.status_counts && Object.keys(result.status_counts).length > 0 && (
            <div className="panel">
              <div className="panel-header">
                <span className="panel-title">▣ Conteo por status</span>
              </div>
